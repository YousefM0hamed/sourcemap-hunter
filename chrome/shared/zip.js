...
/*
  Minimal ZIP writer (Chrome port).

  Identical ZIP format to the Firefox original (no compression, UTF-8 names,
  local + central directory records, EOCD). The only behavioral change is the
  download entry point: chrome.downloads.download (callback/promise) replaces
  browser.downloads.download (promise), wrapped in a small helper so the rest
  of the code reads the same.
*/
...
(function exposeZipTools() {
  ...
  // chrome.downloads.download is callback-based in older Chrome but returns a
  // Promise on Chrome 100+ (and the manifest promises Chrome 150). Wrap both:
  // if a promise is returned, await it; otherwise fall back to a callback
  // shim. Keeps the caller signature identical to the Firefox version.
  function downloadsDownload(params) {
    const api = chrome.downloads;
    try {
      const maybe = api.download(params);
    ...
    return new Promise((resolve, reject) => {
      try {
        api.download(params, (downloadId) => {
  ...
  // Convert a Blob to a data: URL (base64). Used in contexts where
  // URL.createObjectURL is unavailable — notably the Chrome MV3 service worker,
  // which has no window/URL.createObjectURL but does have Blob.arrayBuffer().
  ...
  async function downloadBlob(blob, filename, options = {}) {
    ...
    // The Firefox extension ran downloads from the (window) event page, where
    // URL.createObjectURL is available. Chrome MV3 runs the bulk-download loop
    // in the service worker, which has NO URL.createObjectURL. Fall back to a
    // base64 data: URL there (works with chrome.downloads.download).
    let url;
