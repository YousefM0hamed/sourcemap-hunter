...
const formatToggleEl = document.getElementById("formatToggle");

// The user's choice of format state for the *current* file. Auto-enabled when
// a selected file looks minified; the user can toggle it off to see raw text.
...
// stay identical to the highlighted path. Source-map content is NEVER parsed
// as HTML — it is escaped first and only then placed inside the template.
...
// Returns the resolved language id actually used (for the meta line). Shiki's
// output is static HTML produced from already-escaped tokens; we never feed
// untrusted source-map text into innerHTML except via plainCodeHtml (escaped)
// or Shiki (which escapes internally).
function renderCode(content, language, path) {
  ...
  // Pretty-print minified JS/TS/CSS for readability. Only applied to
  // JS-family/CSS languages (never JSON, HTML, or plain text) and only when the
  // user's Format toggle is on. The beautifier is pure-string (no eval), so
  // untrusted source code can never execute.
  if (
    formatEnabled &&
    globalThis.SourceMapHunterBeautify &&
    (lang === "javascript" || lang === "typescript" || lang === "jsx" || lang === "tsx" || lang === "css")
  ) {
    text = globalThis.SourceMapHunterBeautify.beautify(text, lang);
  }
...
function selectSource(index) {
  ...
  // Auto-enable Format for files that look minified (one giant line). The user
  // can toggle it off to see the raw text. The beautifier is pure-string.
  const content = source.content || "";
  const looksMin =
    globalThis.SourceMapHunterBeautify &&
    globalThis.SourceMapHunterBeautify.looksMinified(content);
  if (looksMin && !formatToggleEl.checked) {
    formatEnabled = true;
    formatToggleEl.checked = true;
  } else if (!looksMin && !formatToggleEl.checked) {
    formatEnabled = false;
  } else {
    formatEnabled = formatToggleEl.checked;
  }

  const usedLang = renderCode(content, source.language, source.path);
...
formatToggleEl.addEventListener("change", () => {
  formatEnabled = formatToggleEl.checked;
  if (currentSource) {
    const usedLang = renderCode(currentSource.content || "", currentSource.language, currentSource.path);
    currentMetaEl.textContent = `${usedLang} · ${formatBytes(currentSource.size)}`;
  }
});
